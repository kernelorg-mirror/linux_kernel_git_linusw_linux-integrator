...
/*
 * Physical start and end address of the kernel. These addresses are
 * 2MB-aligned to match section mappings.
 *
...
#define KERNEL_SECTION_SIZE (kernel_phy_end - kernel_phy_start)
...
static inline unsigned long virt_to_pfn(const void *kaddr)
{
	if (!IS_ENABLED(CONFIG_ARM_KERNEL_IN_VMALLOC)) {
		...
		if ((kaddr >= (void *)KERNEL_OFFSET) &&
		    (kaddr < (void *)(KERNEL_OFFSET + KERNEL_SECTION_SIZE))) {
