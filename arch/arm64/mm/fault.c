			     ...
			     unsigned long esr, struct pt_regs *regs)
{
	...
	unsigned long sync_stack = (unsigned long)raw_cpu_read(sync_stack_ptr);
	bust_spinlocks(1);
	...
	pr_alert("SYNC STACK: 0x%08lx-0x%08lx\n",
		 sync_stack, sync_stack + SYNC_STACK_SIZE - 1);
...
void noinstr do_stack_abort(unsigned long far, struct pt_regs *regs)
{
	unsigned long addr = untagged_addr(far);
	...
	if (!dynamic_stack_fault(current, addr)) {
		...
		die_kernel_fault("dynamic stack fault", addr, esr, regs);
	}
