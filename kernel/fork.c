...
static bool try_release_thread_stack_to_cache(struct vm_struct *vm_area)
{
	...
	scoped_guard(preempt) {
		...
		/*
		 * If we have faulted in more than the default number of
		 * stack pages to a stack, don't re-use it. This makes sure
...
static struct vm_struct *alloc_vmap_stack(int node)
{
	...
	/*
	 * Since the stack grows downward from high addresses, we map
	 * the pages we just obtained from the top of the stack area.
	 *
	 * Notice that we rely on THREAD_SIZE to find the top of the
	 * stack, vm_area->size can not be used because this usually
	 * includes an extra guard page.
	 */
	...
	pr_debug("%s: allocate stack at %lx -> %lx, %d preallocated pages at %lx -> %lx\n",
		 __func__,
...
static int dynamic_stack_refill_pages_cpu(unsigned int cpu)
{
	...
	for (i = 0; i < THREAD_DYNAMIC_PAGES; i++) {
		if (pages[i])
			break;
		pages[i] = alloc_pages(GFP_VMAP_STACK, 0);
		if (unlikely(!pages[i])) {
...
static void dynamic_stack_account_and_alloc_new(struct page **pages, int no_pages)
{
	...
	for (i = 0; i < no_pages; i++) {
		...
		/*
		 * FIXME: Since stack pages were already allocated, we should never
	...
	if (i > 0)
		pr_debug("Allocated %d new pages and accounted\n", i);
}
...
static bool noinstr dynamic_stack_map_page(struct page *page, unsigned long addr)
{
	pte_t *pte;
...
bool noinstr dynamic_stack_fault(struct task_struct *tsk, unsigned long address)
{
	...
	/*
	 * Check if fault address is within the stack hole, illustration
	 * for a stack that grows down, which us currently the only type
	 * of dynamic stack that we support:
	 *
	 * TOP OF STACK, HIGHER ADDRESSES
	 *
	 * +---------------+ stack + THREAD_SIZE |
	 * | mapped page 1 |                     | Stack usage grows down
	 * +---------------+                     |
	 * | mapped page 2 |                     V
	 * +---------------+ hole end
	 * | page hole     |
	 ...
	 * +---------------+ stack
	 *
	 * BASE OF STACK, LOWER ADDRESSES
	 *
	 * vm_area->pages = [ NULL, NULL, mapped page 2, mapped page 1 ]
	 */
	...
	for (addr = hole_end - PAGE_SIZE; addr >= address; addr -= PAGE_SIZE) {
		...
		/* Store the new page in the stack's vm_area, not in any special order */
		vm_area->pages[vm_area->nr_pages] = page;
		vm_area->nr_pages++;

		if (!dynamic_stack_map_page(page, addr))
			return false;
	}
...
static int alloc_thread_stack_node(struct task_struct *tsk, int node)
{
	...
	if (vm_area) {
		...
		if (IS_ENABLED(CONFIG_DYNAMIC_STACK)) {
			/*
			 * Dynamic stack grows from top down so clear the
			 * pages at the top of the address range. We must use
			 * THREAD_SIZE because vm_area->size includes a guard
			 * page.
			 */
			clear_pages(vm_area->addr + THREAD_SIZE -
				    vm_area->nr_pages * PAGE_SIZE,
				    vm_area->nr_pages);
		} else {
			/* The stack is linear and we just clear all pages */
